...
- Task groups: reusable task store, optional stages/subtasks, and per-day todo instances in tracker history. The “Усі завдання” command center derives one grouped view from those existing sources, supports category/priority/status filters, and plans or atomically reschedules daily instances through the same history paths used by the tracker.
