...
- Main: current-day plan-performance dashboard, a mobile daily agenda combining planned habits with concrete task-list items in a touch-scrollable unfinished-plan list, focus carried from yesterday's review, a "never miss twice" recovery cue, quick voice/text task capture with optional time/category into an existing flat task group, copy-yesterday/copy-same-weekday/manual plan setup, inline completion for boolean/measured habits and task items, calculated active-aim progress (including the configured planner-consistency aim for days strictly above 50% plan completion), review status, context-aware empty states, a weekly chart that can exceed 100% for completed work outside the plan, complexity-weighted weekly life balance by habit area, activity streak, full JSON backup download, a water card plus a combined steps/active-calories card, and a one-time callback bridge to a user-created Apple Health Shortcut. The active-calories target is 300 kcal and is shown as guidance rather than a medical recommendation.
