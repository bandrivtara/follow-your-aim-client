...
- Task objects inside task-group definitions and history keep their existing title/status/time shape and may additionally contain an optional `category` using one of the client life-area IDs, an optional `priority` (`low`, `medium`, or `high`), and an optional `failureReason` for failed daily instances. Missing categories, priorities, reasons, and empty times remain valid for legacy and all-day tasks.
...
- The default all-tasks overview reads only taskGroup repositories/stages and excludes completed pool items. Full history is lazy-loaded only for explicit dated/archive filters. Planning creates the same task-group activity shape below a normalized day key; rescheduling uses a Firestore batch so source and destination history paths change atomically, including across month documents. Tracker/dashboard task saves reconcile matching task IDs (or legacy normalized titles) into the existing tasksStore/tasksStages status, and ad-hoc tracker tasks are added to tasksStore. This introduces no collection or date field on task definitions.
