...
- The all-tasks overview reads task repositories/stages plus existing history documents. Planning creates the same task-group activity shape below a normalized day key; rescheduling uses a Firestore batch so source and destination history paths change atomically, including across month documents. It introduces no collection or date field on task definitions.
